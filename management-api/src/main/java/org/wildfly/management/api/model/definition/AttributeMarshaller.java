...
public abstract class AttributeMarshaller {
     ...
     * {@link #isMarshallable(ItemDefinition, ModelNode, boolean) is marshallable}.
     *
     * @param attribute - attribute for which marshaling is being done
     * @param resourceModel the model, a non-null node of {@link org.jboss.dmr.ModelType#OBJECT}.
    ...
    public void marshallAsAttribute(final ItemDefinition attribute, final ModelNode resourceModel, final boolean marshallDefault, final XMLStreamWriter writer) throws XMLStreamException{
        throw ControllerLoggerDuplicate.ROOT_LOGGER.couldNotMarshalAttributeAsAttribute(attribute.getName());
    }
    ...
    public void marshallAsElement(final ItemDefinition attribute, final ModelNode resourceModel, final boolean marshallDefault, final XMLStreamWriter writer) throws XMLStreamException{
        throw ControllerLoggerDuplicate.ROOT_LOGGER.couldNotMarshalAttributeAsElement(attribute.getName());
    }
    ...
    public void marshall(final ItemDefinition attribute, final ModelNode resourceModel, final boolean marshallDefault, final XMLStreamWriter writer) throws XMLStreamException {
    ...
    private static class ObjectMarshaller extends DefaultAttributeMarshaller {
        ...
        private static Set<ItemDefinition> sortAttributes(ItemDefinition[] attributes) {
            Set<ItemDefinition> sortedAttrs = new LinkedHashSet<>(attributes.length);
            List<ItemDefinition> elementAds = null;
            for (ItemDefinition ad : attributes) {
        ...
        public void marshallAsElement(ItemDefinition attribute, ModelNode resourceModel, boolean marshallDefault, XMLStreamWriter writer) throws XMLStreamException {
            ...
            if (resourceModel.hasDefined(attribute.getName())) {
                ItemDefinition[] valueTypes = ((ObjectTypeItemDefinition) attribute).getValueTypes();
                Set<ItemDefinition> sortedAttrs = sortAttributes(valueTypes);
                writer.writeStartElement(attribute.getXmlName());
                for (ItemDefinition valueType : sortedAttrs) {
                    if(resourceModel.hasDefined(attribute.getName(), valueType.getName())) {
                        ...
                        if(marshallSimpleTypeAsElement) {
                            valueType.getMarshaller().marshallAsElement(valueType, handler, marshallDefault, writer);
                        } else {
    ...
    private static class ObjectListMarshaller extends AttributeMarshaller {
        ...
        void writeElements(XMLStreamWriter writer, ObjectTypeItemDefinition objectType, ItemDefinition[] valueTypes, List<ModelNode> elements) throws XMLStreamException {
        ...
        public void marshallAsElement(ItemDefinition attribute, ModelNode resourceModel, boolean marshallDefault, XMLStreamWriter writer) throws XMLStreamException {
            ...
            ItemDefinition[] valueTypes = objectType.getValueTypes();
            if (resourceModel.hasDefined(attribute.getName())) {
