...
public class StringAllowedValuesValidator extends ModelTypeValidator implements AllowedValuesValidator {
    ...
    public StringAllowedValuesValidator(String... values) {
        ...
        List<ModelNode> vals = new ArrayList<>(values.length);
        for (String value : values) {
            vals.add(new ModelNode().set(value));
        }
        allowedValues = Collections.unmodifiableList(vals);
    }
    ...
    public void validateParameter(String parameterName, ModelNode value) throws OperationFailedException {
        ...
        if (value.isDefined()) {
            if (!allowedValues.contains(value)) {
                throw new OperationFailedException(ControllerLoggerDuplicate.ROOT_LOGGER.invalidValue(value.asString(), parameterName, allowedValues));
            }
        }
    }
